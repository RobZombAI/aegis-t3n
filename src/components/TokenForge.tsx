...
import { Coins, Sparkles, Check, Copy, Terminal, Layers } from 'lucide-react'
...
export const TokenForge: React.FC<TokenForgeProps> = () => {
  ...
  const handleDeploy = async (e: React.FormEvent) => {
    ...
    // Simulate deploy transaction on Cookie Chain
  ...
  return (
    <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-slate-800 relative overflow-hidden">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              ...
              Cookie <span className="text-amber-400">Token Forge</span>
            </h3>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Crea e lancia nuovi SPL Token su Cookie Chain SVM con costi inferiori a $0.05
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleDeploy} className="space-y-4">
        ...
        <div className="p-3.5 rounded-xl bg-purple-950/30 border border-purple-800/30 flex items-center justify-between text-xs">
          <span className="text-purple-300 flex items-center gap-1.5">
            <Layers className="w-4 h-4" /> Costo deploy su Cookie Chain:
          </span>
          <span className="font-mono font-bold text-amber-300">
            ~0.002 COOK (~$0.05) vs $2.50+ su Solana
          </span>
        </div>
        ...
        <button
          type="submit"
          disabled={deploying}
          className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold py-3 rounded-xl text-sm shadow-lg shadow-purple-500/20 transition flex items-center justify-center gap-2 disabled:opacity-50"
        >
          {deploying ? (
      ...
      {deployed && (
        <div className="mt-5 p-4 rounded-2xl bg-slate-900/90 border border-purple-500/40 text-slate-200 animate-in fade-in duration-300">
